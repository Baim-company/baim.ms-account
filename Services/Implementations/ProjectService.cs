...
public class ProjectService : IProjectService
{
    ...
    public async Task<Response<Project>> UpdateProjectAsync(UpdateProjectModel projectModel)
    {
        try
        {
            ...
            byte[]? theme = null;
            string themeType = "data:image/png;base64,";
            if (!string.IsNullOrEmpty(projectModel.DesignThemeImage))
            {
                theme = Convert.FromBase64String(projectModel.DesignThemeImage.Split(',')[1]);
                themeType = projectModel.DesignThemeImage.Split(',')[0] + ",";
            }

            byte[]? avatar = null;
            string avatarType = "data:image/png;base64,";
            if (!string.IsNullOrEmpty(projectModel.AvatarImage))
            {
                avatar = Convert.FromBase64String(projectModel.AvatarImage.Split(',')[1]);
                avatarType = projectModel.AvatarImage.Split(',')[0] + ",";
            }
            ...
            projectExist.Id1C = projectModel.Id1C;
            projectExist.DesignTheme = theme;
            projectExist.DesignThemeImageType = themeType;
            projectExist.Avatar = avatar;
            projectExist.AvatarImageType = avatarType;
